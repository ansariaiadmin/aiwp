 ...
 * REST API module.
 *
 * Registers a `store-health/v1` REST namespace. Ships one example read-only
 * route (`/status`) and lets other modules/glue code register more routes
 * via the `ansariai_sh_rest_routes` filter instead of calling
...
namespace AnsariAi\StoreHealth\Modules\RestApi;
...
final class RestApi implements ModuleInterface {
	...
	public const NAMESPACE_V1 = 'store-health/v1';
	...
	public function register_routes(): void {
		 ...
		 */
		$routes = (array) apply_filters( 'ansariai_sh_rest_routes', [] );
		...
		foreach ( $routes as $route ) {
			...
			if ( empty( $route['args']['permission_callback'] ) ) {
				\AnsariAi\StoreHealth\Logger::warning(
