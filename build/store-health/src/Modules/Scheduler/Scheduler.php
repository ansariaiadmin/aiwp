...
namespace AnsariAi\StoreHealth\Modules\Scheduler;
...
final class Scheduler implements ModuleInterface {
	...
	private const GROUP = 'ansariai_sh';
	...
	public function register_custom_intervals( array $schedules ): array {
		$schedules['ansariai_sh_five_minutes'] = [
			'interval' => 5 * MINUTE_IN_SECONDS,
			'display'  => __( 'Every 5 Minutes (Store Health Assistant)', 'store-health' ),
		];
	...
	public function has_action_scheduler(): bool {
		 ...
		 */
		$forced = (string) apply_filters( 'ansariai_sh_scheduler_backend', '' );
	...
	private function closest_wp_cron_recurrence( int $interval_seconds ): string {
		if ( $interval_seconds <= 5 * MINUTE_IN_SECONDS ) {
			return 'ansariai_sh_five_minutes';
		}
