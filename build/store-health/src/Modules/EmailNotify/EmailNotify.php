...
namespace AnsariAi\StoreHealth\Modules\EmailNotify;
...
final class EmailNotify implements ModuleInterface {
	 ...
	 *                                     'ansariai_sh_email_body' filter for template rendering.
	 */
	public function send( string $to, string $subject, string $body_html, array $context = [] ): bool {
		$to = sanitize_email( $to );

		if ( ! is_email( $to ) ) {
			\AnsariAi\StoreHealth\Logger::warning( 'Refused to send email: invalid recipient address.' );
		 ...
		 */
		$subject = (string) apply_filters( 'ansariai_sh_email_subject', $subject, $context );
		 ...
		 * @param array<string,mixed> $context
		 */
		$body_html = (string) apply_filters( 'ansariai_sh_email_body', $body_html, $context );
		 ...
		 */
		$headers = (array) apply_filters( 'ansariai_sh_email_headers', $headers );
		 ...
		 * @param string $subject
		 */
		do_action( 'ansariai_sh_before_email_send', $to, $subject );

		$sent = wp_mail( $to, $subject, $body_html, $headers );

		if ( ! $sent ) {
			\AnsariAi\StoreHealth\Logger::error( 'wp_mail() returned false.', [ 'to_domain' => substr( (string) strrchr( $to, '@' ), 1 ) ] );
		}
