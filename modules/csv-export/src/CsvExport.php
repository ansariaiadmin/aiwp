...
namespace VendorPlugin\Modules\CsvExport;
...
final class CsvExport implements ModuleInterface {
	...
	public function register(): void {
		add_action( 'admin_post_{{PREFIX}}_export_csv', [ $this, 'handle_download' ] );
	}
	...
	public function handle_download(): void {
		Guard::verify_write( '{{PREFIX}}_export_csv', '_wpnonce', 'manage_options' );
		 ...
		 */
		$rows = (array) apply_filters( '{{PREFIX}}_csv_export_rows', [] );

		$filename = sanitize_file_name( '{{SLUG}}-export-' . gmdate( 'Y-m-d' ) . '.csv' );
