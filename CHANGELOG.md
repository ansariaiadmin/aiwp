...
- **Hard review rules enforced at validate time** (`tools/validate.php`): `wporg=true` rejects the `license-client` module and `license.enabled=true` (no commercial gating on wp.org), and requires ≥3 valid lowercase-hyphenated readme tags.
- **Review-ready `readme.txt` generation** (`tools/compose.php`): real Tags, full Description (from `spec.readme.longDescription`), custom FAQ entries, Changelog and GPL license header — instead of the minimal stub used for private builds.
- **Commercial branding stripped from wp.org builds**: no vendor domain or "generated by" marketing lines leak into the plugin header.
- **`spec/examples/store-health-wporg.json`**: ready-to-build wp.org example spec.
- **6 new tests** covering the wp.org mode (suite: 25 → 31, all green on native-style runner and php-wasm).
