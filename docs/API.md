...
curl -X POST http://localhost:3000/api/auth/login \
  ...
  -d '{"email":"admin@aiwp.dev","password":"Admin@123"}'
```
...
{
  "token": "jwt-token-...",
  "user": { "email": "admin@aiwp.dev", "role": "admin" }
}
```
...
```json
{
  ...
  "providers": {
    ...
    "telegram": { "status": "ok", "bot": "@aiwp_notif_bot" }
  },
  "notifications": {
...
1. تلگرام → @BotFather → /newbot → اسم → یوزرنیم (مثل aiwp_notif_bot)
