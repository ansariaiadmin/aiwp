...
- `license-client` and `license.enabled=true` are **rejected** — wp.org
  plugins must not gate features or updates behind a license key.
- `readme.tags` must contain **at least 3** lowercase, hyphenated tags
  (single auto-generated tags get rejected by directory review).
- `readme.txt` is rendered with real Tags, a full Description
  (`readme.longDescription`), custom FAQs (`readme.faqs`), a Changelog
  section and the GPLv2 license header.
- Commercial branding (vendor domain, "generated by …" lines) is stripped
  from the main plugin file header.

`spec/examples/store-health-wporg.json` is a complete, buildable reference
for this mode: `php tools/build.php spec/examples/store-health-wporg.json`.
