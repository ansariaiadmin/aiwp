# راهنمای کامل AiWp — WordPress Plugin Factory + SaaS License Platform — برای افراد غیر فنی
...
## 🧭 راهنمای تصویری پلاگین ساخته‌شده (Help Guide)

هر فیلد در صفحه تنظیمات پلاگین، یک دکمه علامت سؤال **(؟)** کنار خودش دارد. با کلیک روی آن، یک پنجره راهنمای زیبا باز می‌شود که شامل سه بخش است:

1. **تصویر توضیحی (SVG):** یک آیکون گرافیکی مرتبط (مثلاً ✉️ برای ایمیل، 📊 برای چارت، 🔔 برای نوتیفیکیشن) بالای پنجره نمایش داده می‌شود.
2. **متن راهنمای کامل:** توضیح دقیق و مرحله‌به‌مرحله آن قابلیت.
3. **مثال واقعی:** مقدار نمونه که باید وارد کنید.

**ویژگی‌های انیمیشنی:**
- باز شدن نرم با افکت Fade-in + Scale
- بسته شدن با کلید `Esc` یا کلیک بیرون پنجره
- فقط یک پنجره همزمان باز می‌ماند
- پشتیبانی کامل RTL و موبایل

**نحوه تعریف راهنما در Spec فایل:**
در فایل JSON پلاگین خود، برای هر option این سه کلید را اضافه کنید:
```json
{
  "key": "report_email",
  "help": "آدرس ایمیل مقصد گزارش‌ها... <b>حداقل یک ایمیل</b>",
  "example": "admin@example.com",
  "illustration": "email"
}
```
مقادیر مجاز `illustration`: `email`, `bell`, `lock`, `chart`, `clock`, `database`, `globe`, `shield`, `sparkles`, `list`
...
- ایشو باز کن: https://github.com/ansariaiadmin/aiwp/issues
