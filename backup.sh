...
BACKUP_FILE="$BACKUP_DIR/aiwp-backup-$TIMESTAMP.tar.gz"
BACKUP_ENCRYPTED="$BACKUP_DIR/aiwp-backup-$TIMESTAMP.tar.gz.enc"
...
if command -v docker &> /dev/null && docker compose ps 2>/dev/null | grep -q "Up"; then
  ...
  docker compose exec -T db pg_dump -U aiwp aiwp 2>/dev/null > /tmp/aiwp-db-$TIMESTAMP.sql || echo "DB dump fail — شاید DB خاموشه"
  ok "DB dump — /tmp/aiwp-db-$TIMESTAMP.sql"
else
...
if [ -f /tmp/aiwp-db-$TIMESTAMP.sql ]; then
  tar -rf $BACKUP_FILE /tmp/aiwp-db-$TIMESTAMP.sql 2>/dev/null || true
...
ls -t $BACKUP_DIR/aiwp-backup-*.tar.gz* 2>/dev/null | tail -n +8 | xargs rm -f 2>/dev/null || true
